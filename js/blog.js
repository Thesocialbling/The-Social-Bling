...
function openArticle(id) {
  document.getElementById('blx-listing').style.display = 'none';
  document.querySelectorAll('.pg-article').forEach(a => a.classList.remove('on'));
  document.getElementById(id).classList.add('on');
  window.scrollTo({ top: 0, behavior: 'smooth' });
}
function closeArticle() {
  document.querySelectorAll('.pg-article').forEach(a => a.classList.remove('on'));
  document.getElementById('blx-listing').style.display = '';
  window.scrollTo({ top: 0, behavior: 'smooth' });
}
...
function blxF(btn, cat) {
  document.querySelectorAll('#blxFilters .blx-fbtn').forEach(b => b.classList.remove('on'));
  btn.classList.add('on');
  const feat = document.querySelector('.blx-feat');
  const cards = document.querySelectorAll('#blx-cards .blx-c');
  [feat, ...cards].forEach(c => {
    if (!c) return;
    const show = cat === 'all' || c.dataset.bcat === cat;
    if (show) {
      c.style.display = '';
      c.style.opacity = '0'; c.style.transform = 'translateY(14px)';
      setTimeout(() => { c.style.transition = 'opacity .4s,transform .4s'; c.style.opacity = '1'; c.style.transform = 'translateY(0)'; }, 20);
