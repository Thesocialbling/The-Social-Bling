...
// ── FOUNDERS DATA — edit names, posts, messages here ──
const FOUNDERS = [
  {
    initial: 'A',
    name: 'Founder One',
    ...
    accentColor: '#FFC6D3',
    message: "I started Social Bling because I was tired of watching brilliant founders get mediocre results from agencies that didn't actually care. Every brand has a story that deserves to be told boldly. My job is to find that story and make the whole world impossible to look away from it. We built this studio for founders who believe their brand can genuinely change how people feel — not just what they buy.",
    tags: ['Creative Direction', 'Brand Strategy', 'Visual Identity', 'Culture Marketing']
  },
  {
    initial: 'B',
    name: 'Founder Two',
    post: 'Co-Founder · Brand Architect',
    role: 'Co-Founder',
    bgGradient: 'linear-gradient(135deg,#0d1a14,#1a1410)',
    accentColor: '#4a8a6a',
    message: "Great brands aren't built by committees. They're built by people who are unreasonably obsessed with getting it right. I bring the architecture — the structure, the strategy, the systems that make a brand work at scale. When creative genius meets business precision, that's where magic actually happens. That's what we built Social Bling to be.",
    tags: ['Brand Architecture', 'Growth Strategy', 'Content Systems', 'Global Expansion']
  }
];
...
function openFounder(idx) {
  ...
  document.getElementById('modalInitial').textContent = f.initial;
  document.getElementById('modalInitial').style.background = f.accentColor === '#FFC6D3'
    ? 'linear-gradient(135deg,#FFC6D3,#FFB3C6)' : 'linear-gradient(135deg,#6b9e6b,#4a8fa8)';
  document.getElementById('modalInitial').style['-webkit-background-clip'] = 'text';
  document.getElementById('modalInitial').style['background-clip'] = 'text';
  document.getElementById('modalInitial').style['-webkit-text-fill-color'] = 'transparent';
